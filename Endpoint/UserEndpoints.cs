...
public static class UserEndpoints
{
	public static void MapUserEndpoints(this IEndpointRouteBuilder routes)
	{
		...
		//Completed
		group.MapPost("/Register", (Register user, Backend_ASP_ProjectContext db) =>
		{
			var checkUsername = db.Users.FirstOrDefault(item => item.Username == user.Username && item.IsDelete == 0);
			...
			if (damon_Tool.CheckValidate(typeof(Register), user) != "")
			{
				postRegisterResponse.Message = damon_Tool.CheckValidate(typeof(Register), user);
			...
			if (checkUsername == null)
			{
				...
				{
					postRegisterResponse.Message = "Password and ConfirmPassword are not Matched!";
					return postRegisterResponse;
				}
			}
			else
				...
				if (Token != null)
				{
					if (db.Users.FirstOrDefault(user => user.TokenId == Token.Id && user.IsDelete == 0) != null)
					{
						if (Params.Password == Params.ConfirmPassword)
						{
							...
							{
								var data = db.Users.First(user => user.Id.ToString() == Params.Id && user.IsDelete == 0);
